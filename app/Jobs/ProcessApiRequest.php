...
class ProcessApiRequest implements ShouldQueue
{
    ...
    public function handle()
    {
        ...
        foreach ($json_data as $item) {
            if (isset($item['customFields'])) {
                foreach ($item['customFields'] as $field) {
                    if (isset($field['name']) && $field['name'] === 'My Utilities Ordered') {
                        if (isset($field['values']) && is_array($field['values'])) {
                            if (isset($field['values'][0])) {
                                $value = $field['values'][0];

                                if ($value === true) {
                                    Log::info('My Utilities Ordered is true');

                                    $this->sendDataToApi($json_data);

                                    return;
                                } elseif ($value === false) {
                                    Log::error('My Utilities Ordered is not true');
                                    return;
                                } else {
                                    Log::error('Unexpected value for My Utilities Ordered');
                                    return;
                                }
                            } else {
                                Log::error('No values found for My Utilities Ordered');
                                return;
                            }
                        } else {
                            Log::error('The "values" key is not set or is not an array');
                            return;
                        }
                    }
                }
        ...
        Log::error('My Utilities Ordered field not found in customFields');
    }

    protected function sendDataToApi($json_data)
    {
        try {
            $borrower = $json_data[0]['borrowers'][0];
